...
const llmDecisionSchema = z.object({
  ...
  params: z.object({
    ...
    originalRequestNature: z.enum(["singular", "plural_or_unspecified"]).nullable().optional(), // Hint for deletion requests
...
function formatConversationHistoryForPrompt(history: ConversationTurn[]): string {
  ...
  return history
    .map(turn => {
      let messagePrefix = `${turn.actor === 'USER' ? 'User' : 'Assistant'}:`;
      let messageText = turn.messageText;

      if (turn.actor === 'ASSISTANT' && turn.toolCalled?.startsWith('planner->create_event') && turn.toolResult && Array.isArray(turn.toolResult)) {
        const createdEvents = turn.toolResult as Array<any>; // Assuming this matches CreatedEventDetails structure
        if (createdEvents.length > 0) {
          const eventSummaries = createdEvents.map(event => {
            let summary = event.summary || 'Unnamed Event';
...
export async function getNextAction(
  ...
  if (activeClarificationContext && activeClarificationContext.type === 'delete_candidates_for_confirmation') {
    ...
    4.  **Ask for More Specific Clarification (Rare):** If the response is extremely vague and you cannot determine any of the above, you can use 'ask_user_question'.
  ...
  } else if (isClarificationRequest && currentUserMessage.includes("SYSTEM_CLARIFICATION_REQUEST: Type: delete_clarify_time_range")) {
    // Mode: Formulate a clarification question for deletion time range
    // Extract details from currentUserMessage which contains JSON.stringify(executePlanResult for time range)
    ...
    try {
      ...
      console.error("[CentralOrchestratorLLM] Error parsing time range clarification details:", e);
    }
  ...
  try {
    ...
    let finalParams = validatedDecision.params || {}; // Ensure finalParams is an object
